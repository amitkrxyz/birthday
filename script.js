...
setInterval(() => {
  ...
  if (
    ...
    if (
      ...
      nextBday = new Date(
        `${dob.getMonth() + 1}/${dob.getDate()}/${nowDate.getFullYear() + 1}`
      );
    } else if (
      (dob.getDate() < nowDate.getDate() &&
        dob.getMonth() > nowDate.getMonth()) ||
...
function addBox() {
  const box = Bodies.polygon(
    ...
    0 - SIZE,
    Math.round(Common.random(2, 8)),
    SIZE,
    { restitution: 0.8, friction: 0.8 }
  );
